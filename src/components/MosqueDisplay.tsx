...
export function MosqueDisplay({
  ...
  return (
    <div
      className={`flex flex-col min-h-screen lg:h-screen lg:overflow-hidden pb-[clamp(2.5rem,4vw,4rem)] transition-colors duration-300 ${d ? 'bg-[#0f1c1e] islamic-pattern-dark' : 'bg-[#f2f8f8] islamic-pattern-light'}`}
    >
      {/* Two-column layout */}
      <div className='flex flex-col lg:flex-row flex-1 min-h-0'>
        ...
        <div className='w-full lg:w-[55%] flex flex-col px-[clamp(1rem,3vw,3rem)] py-[clamp(1rem,2vw,2rem)] min-h-0'>
          <div className='flex flex-col min-h-0 flex-1'>
            ...
            <div className='flex justify-between gap-4 items-start'>
              <div>
                ...
                <p
                  className={`text-[clamp(0.95rem,1.3vw,1.5rem)] font-semibold transition-colors ${d ? 'text-[#f0ebe0]' : 'text-[#1a3035]'}`}
                >
                  {format(now, 'EEEE, d MMMM yyyy')}
                </p>
                {hijri && (
                  <p
                    className={`text-[clamp(0.8rem,0.95vw,1.1rem)] font-medium mt-0.5 transition-colors ${d ? 'text-[#e8c97e]' : 'text-[#9b6f1a]'}`}
                  >
                ...
                {nextHoliday && nextHoliday.daysUntil >= 0 && (
                  <p
                    className={`text-[clamp(0.65rem,0.8vw,0.85rem)] font-medium mt-0.5 transition-colors ${d ? 'text-[#e8c97e]/60' : 'text-[#9b6f1a]/60'}`}
                  >
                ...
                {method && (
                  <p
                    className={`text-[clamp(0.65rem,0.8vw,0.85rem)] mt-2 font-medium tracking-wide transition-colors ${d ? 'text-[#7ba8ac]' : 'text-[#5a8a8e]'}`}
                  >
                    {method}
                  </p>
                )}
          ...
          <div className='lg:hidden w-full mt-[clamp(1rem,2vw,2rem)] flex flex-col gap-[clamp(0.4rem,1vw,0.75rem)]'>
            {prayers.map((prayer) => (
              <PrayerColumn key={prayer.key} prayer={prayer} isNext={prayer.key === nextPrayer?.key} isDark={d} />
            ))}
          </div>
        </div>
        ...
        <div
          className={`hidden lg:flex w-full lg:w-[45%] flex-col justify-center gap-[clamp(0.35rem,1vw,1rem)] px-[clamp(1rem,2vw,3rem)] py-[clamp(1rem,2vw,2rem)] min-h-0 transition-colors ${d ? 'border-[#11999e]/15' : 'border-[#11999e]/12'}`}
        >
          {prayers.map((prayer) => (
            <PrayerColumn key={prayer.key} prayer={prayer} isNext={prayer.key === nextPrayer?.key} isDark={d} />
          ))}
      ...
      {syncing && syncProgress !== undefined && syncTotal !== undefined && (
        <div
          className={`w-full px-4 py-1 flex items-center gap-3 transition-colors ${d ? 'bg-[#0a1618]' : 'bg-[#e4f2f2]'}`}
        >
          ...
          <span
            className={`text-[0.65rem] whitespace-nowrap transition-colors ${d ? 'text-[#7ba8ac]' : 'text-[#5a8a8e]'}`}
          >
      ...
      {/* Bottom-left buttons */}
      <div className='fixed bottom-[calc(clamp(2.5rem,4vw,4rem)+4px)] left-4 flex items-center gap-2 z-50'>
        {/* Theme toggle */}
        <button
