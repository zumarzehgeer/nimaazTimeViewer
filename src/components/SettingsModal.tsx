...
// Module-level constants — safe to reference from module-level components
const sectionLabelCls = 'text-xs font-bold text-[#11999e] uppercase tracking-widest block'

// Defined outside SettingsModal so React never remounts them on re-render
function SectionCard({ title, children, action }: { title: string; children: React.ReactNode; action?: React.ReactNode }) {
  return (
    <div className="bg-white/60 rounded-xl border border-[#11999e]/10 shadow-sm p-4">
...
export function SettingsModal({ settings, onSave, onClose, onSignOut }: SettingsModalProps) {
  ...
  const inputCls = 'w-full px-3 py-2 border border-[#11999e]/20 rounded-lg text-sm bg-white/70 text-[#3c3c3c] placeholder-[#3c3c3c]/40 focus:outline-none focus:ring-2 focus:ring-[#11999e]/40 focus:border-[#11999e]/50 transition-colors'
  const subLabelCls = 'text-xs font-semibold text-[#3c3c3c]/60 uppercase tracking-wide mb-1.5 block'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-3 sm:p-4 animate-backdrop-enter">
      <div className="bg-[#FFF8F0] rounded-2xl shadow-2xl w-full max-w-[680px] max-h-[92vh] flex flex-col border border-[#11999e]/10 animate-modal-enter overflow-hidden">
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden relative">
          ...
          {/* Desktop sidebar */}
          <nav className="hidden md:flex flex-col w-44 flex-shrink-0 bg-[#FFF3E8] border-r border-[#11999e]/10 py-3 gap-0.5 px-2">
            {TABS.map((tab) => (
              <button
                key={tab.key}
                onClick={() => setActiveTab(tab.key)}
                className={`flex items-center gap-2.5 w-full px-3 py-2.5 rounded-xl text-left text-sm font-medium transition-all
                  ${activeTab === tab.key ? 'bg-[#11999e] text-white shadow-sm' : 'text-[#3c3c3c]/60 hover:text-[#3c3c3c] hover:bg-[#11999e]/10'}`}
              >
                <tab.Icon size={16} strokeWidth={activeTab === tab.key ? 2 : 1.75} className="flex-shrink-0" />
                <span className="truncate">{tab.label}</span>
              </button>
            ))}
            <div className="mt-auto px-3 pt-3 border-t border-[#11999e]/10">
              <p className="text-[10px] text-[#3c3c3c]/30 font-medium tracking-wide">Nimaaz Time</p>
            </div>
          </nav>

          {/* Mobile top tab strip */}
          <div className="md:hidden absolute top-0 left-0 right-0 z-10 bg-[#FFF3E8] border-b border-[#11999e]/10">
              ...
              {TABS.map((tab) => (
                <button
                  ...
                  className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all
                    ${activeTab === tab.key ? 'bg-[#11999e] text-white' : 'text-[#3c3c3c]/50 hover:text-[#3c3c3c]'}`}
                >
                  <tab.Icon size={13} />
                  {tab.label}
                </button>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 md:pt-4 pt-14 min-h-0 flex flex-col gap-4">
            ...
            {activeTab === 'general' && (
              <>
                ...
                <SectionCard title="Location">
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="px-3 py-2 border border-[#11999e]/20 rounded-lg bg-white/70 hover:bg-[#11999e]/10 text-[#11999e] disabled:opacity-50 transition-colors"
                    >
                      {detecting
                  ...
                  {cityResults.length > 0 && (
                    <ul className="mt-1.5 border border-[#11999e]/15 rounded-xl overflow-hidden shadow-sm">
                      {cityResults.map((r) => (
                        <li
                          key={r.place_id}
                          onMouseDown={() => handleCitySelect(r)}
                          className="px-3 py-2 text-sm hover:bg-[#11999e]/8 cursor-pointer truncate text-[#3c3c3c] transition-colors"
                        >
                          {r.display_name}
                        </li>
                      ))}
                ...
                <SectionCard title="Calculation Method">
                  ...
                  {loadingMethods && (
                    <p className="mt-1.5 text-xs text-[#3c3c3c]/40 flex items-center gap-1.5">
            ...
            {activeTab === 'prayers' && (
              <>
                <SectionCard title="Iqamah Offsets">
                  <p className="text-xs text-[#3c3c3c]/40 mb-3 -mt-1">Minutes after Adhan for each prayer</p>
                  <div className="grid grid-cols-2 gap-2">
                    {(['Fajr', 'Dhuhr', 'Asr', 'Maghrib', 'Isha'] as const).map((key) => (
                      <label key={key} className="flex items-center gap-2.5 bg-[#FFF8F0] rounded-lg px-3 py-2 border border-[#11999e]/10">
                        ...
                        <input
                          ...
                          className="w-14 text-center px-2 py-1 border border-[#11999e]/20 rounded-md text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#11999e]/30 text-[#3c3c3c]"
                        />
                        <span className="text-xs text-[#3c3c3c]/40">min</span>
                      </label>
                    ))}
                  </div>
                </SectionCard>

                <SectionCard title="Jumu'ah">
                  <div className="flex flex-col gap-2.5">
                    <label className="flex items-center gap-3 bg-[#FFF8F0] rounded-lg px-3 py-2.5 border border-[#11999e]/10">
                      ...
                      <input
                        ...
                        className="px-2 py-1 border border-[#11999e]/20 rounded-md text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#11999e]/30 text-[#3c3c3c]"
                      />
                      {!draft.jumuahAdhan && (
                        <span className="text-xs text-[#3c3c3c]/40 italic">defaults to Dhuhr</span>
                      )}
                    </label>
                    <label className="flex items-center gap-3 bg-[#FFF8F0] rounded-lg px-3 py-2.5 border border-[#11999e]/10">
                      ...
                      <input
                        ...
                        className="w-16 text-center px-2 py-1 border border-[#11999e]/20 rounded-md text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#11999e]/30 text-[#3c3c3c]"
                      />
                      <span className="text-xs text-[#3c3c3c]/40">min</span>
                    </label>
                  </div>
              ...
              <SectionCard title="Announcements">
                <div className="flex gap-2 mb-3">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-[#11999e] text-white text-sm font-medium rounded-lg hover:bg-[#0d7a7f] transition-colors flex-shrink-0"
                  >
                    Add
                  </button>
                </div>
                {draft.announcements.length === 0 ? (
                  <p className="text-xs text-[#3c3c3c]/30 text-center py-4 italic">No announcements yet</p>
                ) : (
                  <ul className="space-y-1.5 max-h-48 overflow-y-auto">
                    {draft.announcements.map((a, i) => (
                      <li
                        key={i}
                        className="flex items-center gap-2 text-sm text-[#3c3c3c] bg-[#FFF8F0] border border-[#11999e]/10 px-3 py-2 rounded-lg"
                      >
                        <IconSpeakerphone size={13} className="text-[#11999e] flex-shrink-0" />
                        <span className="flex-1 truncate">{a}</span>
                        <button
                          onClick={() => removeAnnouncement(i)}
                          className="text-[#3c3c3c]/30 hover:text-red-400 transition-colors flex-shrink-0"
                        >
                          <IconX size={14} />
                        </button>
                      </li>
            ...
            {activeTab === 'hadith' && (
              <>
                {/* Enable toggle card */}
                <div className="bg-white/60 rounded-xl border border-[#11999e]/10 shadow-sm p-4 flex items-center justify-between">
                  <div>
                    <span className="text-xs font-bold text-[#11999e] uppercase tracking-widest block mb-0.5">Daily Hadith</span>
                    <p className="text-xs text-[#3c3c3c]/40">Show a rotating hadith on the display</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-[#3c3c3c]/40">{draft.hadith.enabled ? 'On' : 'Off'}</span>
                    <Toggle
                      checked={draft.hadith.enabled}
                      onChange={(v) => setDraft((d) => ({ ...d, hadith: { ...d.hadith, enabled: v } }))}
                    />
                  </div>
                </div>
                  ...
                  <>
                    ...
                    <SectionCard title="Display Options">
                      <div className="flex items-center justify-between py-1">
                        <div>
                          <span className="text-sm font-medium text-[#3c3c3c]">Show Arabic Text</span>
                          <p className="text-xs text-[#3c3c3c]/40 mt-0.5">Display original Arabic alongside translation</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-[#3c3c3c]/40">{draft.hadith.showArabic ? 'On' : 'Off'}</span>
                          <Toggle
                            checked={draft.hadith.showArabic}
                            onChange={(v) => setDraft((d) => ({ ...d, hadith: { ...d.hadith, showArabic: v } }))}
                          />
                        </div>
                      </div>
                    </SectionCard>
                        ...
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <button
                              ...
                              className="absolute right-3 top-1/2 -translate-y-1/2 text-[#3c3c3c]/40 hover:text-[#11999e] transition-colors"
                            >
                              {showApiKey ? <IconEyeOff size={16} /> : <IconEye size={16} />}
                            </button>
                          </div>
                          <p className="mt-1.5 text-xs text-[#3c3c3c]/40">
                            Free key at{' '}
                    ...
                    {/* Pin Specific Hadith */}
                    <SectionCard
                      title="Pin Specific Hadith"
                      action={
                        draft.hadith.pinnedHadith ? (
                          <button onClick={handlePinClear} className="text-xs text-red-400 hover:text-red-500 font-medium transition-colors">
                            Clear pin
                          </button>
                        ) : undefined
                      }
                    >
                      ...
                      <div className="flex gap-2 mb-2">
                        <select
                          ...
                          className="flex-1 px-2 py-1.5 border border-[#11999e]/20 rounded-lg text-xs bg-white/70 focus:outline-none focus:ring-2 focus:ring-[#11999e]/30 text-[#3c3c3c] cursor-pointer"
                        >
                        ...
                        <input
                          ...
                          className="w-24 px-2 py-1.5 border border-[#11999e]/20 rounded-lg text-xs text-center bg-white/70 focus:outline-none focus:ring-2 focus:ring-[#11999e]/30 text-[#3c3c3c]"
                        />
                        <button
                      ...
                      {pinPreview && (
                        <div className="bg-[#FFF8F0] rounded-xl p-3 mb-2 border border-[#11999e]/15">
                          <p className="text-xs text-[#11999e] font-semibold mb-1">{pinPreview.source} #{pinPreview.number}</p>
                          <p className="text-xs text-[#3c3c3c]/70 line-clamp-3 leading-relaxed">{pinPreview.english}</p>
                          <button
                            onClick={handlePinSave}
                            className="mt-2.5 px-3 py-1.5 bg-[#11999e] text-white text-xs rounded-lg hover:bg-[#0d7a7f] font-medium transition-colors"
                          >
                            Pin this hadith
                          </button>
                        </div>
                      )}
                      <p className="text-xs text-[#3c3c3c]/35 leading-relaxed">When pinned, rotation pauses and this hadith always shows.</p>
                    </SectionCard>

                    {/* Collections */}
                    <SectionCard title="Enabled Collections">
                          ...
                          return (
                            <label
                              ...
                              className={`flex items-center gap-2 cursor-pointer px-3 py-2 rounded-lg border transition-all
                                ${isChecked
                                  ? 'border-[#11999e]/30 bg-[#11999e]/8 text-[#11999e]'
                                  : 'border-[#3c3c3c]/10 bg-white/50 text-[#3c3c3c]/50 hover:border-[#11999e]/15'
                                }`}
                            >
                              <input
                                type="checkbox"
                      ...
                      {collectionsError && (
                        <p className="mt-2 text-xs text-red-400">{collectionsError}</p>
                      )}
                    </SectionCard>
                  </>
                )}
            ...
            {activeTab === 'account' && (
              <SectionCard title="Account">
                <p className="text-xs text-[#3c3c3c]/40 mb-4 -mt-1">Manage your mosque account</p>
                {onSignOut ? (
                  <button
                    onClick={onSignOut}
                    className="w-full py-2.5 rounded-xl text-sm font-semibold text-red-500 border border-red-200/60 bg-red-50/60 hover:bg-red-100/60 hover:border-red-300/60 transition-colors"
                  >
                    Sign Out
                  </button>
                ) : (
                  <p className="text-xs text-[#3c3c3c]/30 text-center py-4 italic">Not signed in</p>
                )}
              </SectionCard>
            )}
        ...
        {/* Footer: Save / Cancel */}
        <div className="flex-shrink-0 px-4 py-3.5 border-t border-[#11999e]/10 bg-[#FFF3E8] rounded-b-2xl flex gap-2.5">
          <button
            onClick={onClose}
            className="flex-1 px-4 py-2.5 border border-[#3c3c3c]/15 rounded-xl text-sm font-medium text-[#3c3c3c]/60 hover:text-[#3c3c3c] hover:bg-white/50 transition-colors"
          >
            Cancel
          </button>
          <button
