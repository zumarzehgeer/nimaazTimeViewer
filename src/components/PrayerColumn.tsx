  ...
  return (
    <div
      className={`
        ...
        px-[clamp(0.75rem,1.5vw,2rem)] py-[clamp(0.35rem,0.7vw,0.85rem)]
        transition-all
        ${isNext ? nextCard : `${baseCard} opacity-70`}
      `}
    >
      {/* Icon */}
      <div className="w-[clamp(2rem,3vw,3.5rem)] flex justify-center flex-shrink-0">
        {(() => {
          const Icon = ICONS[prayer.key]
          return Icon
            ? <Icon size="clamp(1.5rem,2.5vw,3rem)" strokeWidth={1.5} className={isNext ? 'text-white' : 'text-[#11999e]'} />
            : null
        })()}
      </div>

      {/* Prayer name */}
      <p className={`text-[clamp(0.75rem,1vw,1.25rem)] font-semibold uppercase tracking-widest w-[clamp(4rem,6vw,7rem)] flex-shrink-0 ${isNext ? 'text-white' : 'text-[#3c3c3c]'}`}>
        {prayer.name}
      </p>
      ...
      <div className="flex-1 text-center">
        {!NO_ADHAN_KEYS.has(prayer.key) && (
          <p className={`text-[clamp(9px,0.6vw,0.7rem)] uppercase tracking-wider ${isNext ? 'text-white/70' : 'text-[#3c3c3c]/40'}`}>Adhan</p>
        )}
        <p className={`text-[clamp(1rem,1.8vw,2rem)] font-bold tabular-nums ${isNext ? 'text-white' : 'text-[#3c3c3c]'}`}>
      ...
      <div className="flex-1 text-center">
        {prayer.iqamahTime ? (
          <>
            <p className={`text-[clamp(9px,0.6vw,0.7rem)] uppercase tracking-wider ${isNext ? 'text-white/70' : 'text-[#3c3c3c]/40'}`}>Iqamah</p>
            <p className={`text-[clamp(1rem,1.8vw,2rem)] font-semibold tabular-nums ${isNext ? 'text-[#6decb9]' : 'text-[#11999e]'}`}>
